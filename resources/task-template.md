...
> task 是 plan.md conventions 应用决策的具体落地：plan 中确定的命名规则、存储格式、参数档位等须硬编码到代码中。
...
spec_ref: "（对应 spec.md 中的条目编号，如 spec#3.2）"
...
简要说明本 task 做什么、为什么做，业务背景一句话概括。

#### 完整可执行代码

> ⚠️ conventions 参数（表名前缀、存储格式、Spark 资源参数等）必须内联到代码中，不得以「参见 conventions」替代。
...
```sql
-- 创建 DWD 层用户行为宽表
-- 依据 spec#3.2，conventions: project-conventions v1.2，ORC+SNAPPY，按 p_date 分区
...
-- 执行引擎：Spark SQL，队列：queue_dw_prod
-- 依据 spec#3.2，conventions: platform-conventions v2.0，executor×10/4g/2core
